...
sampleRate_(DEFAULT_SAMPLE_RATE)
{
    ...
    PrintIfErr( MIDIClientCreate(CFSTR("AudioUnitHost"), MIKMIDIDeviceManagerNotifyCallback, NULL, &client_) );
    PrintIfErr( MIDIInputPortCreate(client_,
                                    CFSTR("InputPort"),
                                    MidiReadProc,
                                    this,
                                    &inputPort_ ) );
    cout << "\n------- MIDI INPUT SOURCES  -------" << endl << endl;
    for(int i=0; i<int(MIDIGetNumberOfSources()); i++) {
        MIDIEndpointRef sourceRef = MIDIGetSource(i);
        CFStringRef name = nil;
        MIDIObjectGetStringProperty(sourceRef, kMIDIPropertyDisplayName, &name );
        PrintCFStringRef(name);
        cout << "\n ";
        
        PrintIfErr( MIDIPortConnectSource(inputPort_, sourceRef, NULL) );
