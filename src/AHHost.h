...
public:
    ...
    vector<AHTrack*>& GetTracks() {
        return tracks_;
    }
    
    AHMidiPlayer* GetAHMidiPlayer() {
        return &midiPlayer_;
    }
    AHGraph* GetAHGraph() {
        return &graph_;
    }
    void BounceToFile( CFURLRef outputFilePathUrl,OSType dataFormat,AudioFileTypeID destFileType );
    
    void ListenToMidi();
    void StopListeningToMidi();
    Boolean IsListeningToMidi() {
        return listeningToMidi_;
    }
